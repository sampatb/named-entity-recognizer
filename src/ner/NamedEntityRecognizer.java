   ...
   */
	public void recognize(final String filename) {
		...
		try {
		    ...
		    _special_symbols.add(",");_special_symbols.add(".");_special_symbols.add("-");
		    _special_symbols.add("\"");_special_symbols.add("(");_special_symbols.add(")");
		    _special_symbols.add("--");_special_symbols.add(":");_special_symbols.add("-X-");
		    _special_symbols.add("$");_special_symbols.add("''");
			...
			*/
			...
			for(int i=4; i<7; i++) {
				...
				buffer[i] = str;
				String s[] = str.split(" ");
				if(s.length > 0) windowBuffer[i] = s[0];
				else windowBuffer[i] = new String();
			}
			...
			while(count != 0) {
				...
				if(!flag && ((str=brAhead.readLine()) != null)) {
					buffer[6] = str;
					String s[] = str.split(" ");
					if(s.length > 0) windowBuffer[6] = s[0];
					else windowBuffer[6] = new String();
				}
				else {
					buffer[6] = new String();
